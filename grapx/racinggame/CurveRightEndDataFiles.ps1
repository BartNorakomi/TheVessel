$outputFile = "CurveRightEndDataFiles.asm"
$startFrame = 1
$endFrame = 75
$RomBlockSize = 16KB
...
function Write-BlockHeader($blockNum) {
    @"
CurveRightEndBlock${blockNum}:  			equ   (`$-RomStartAddress) and (romsize-1) / RomBlockSize
	phase 0x8000
...
for ($i = $startFrame; $i -le $endFrame; $i++) {
    ...
    foreach ($page in 0, 1) {
        foreach ($region in "TOP", "BOTTOM") {
            # Build file paths
            $changedPixelsFile = ".\CurveRightEndAnimationPage${page}\${region}changedpixelsfrom${i}to${j}.bin"
            $addressesFile     = ".\CurveRightEndAnimationPage${page}\${region}addressesfrom${i}to${j}.bin"
            $writeInstrFile    = ".\CurveRightEndAnimationPage${page}\${region}writeinstructionsfrom${i}to${j}.bin"
            ...
            $blockLabel = "BlockCurveRightEndPage${page}${region}from${i}to${j}"
            $prefix     = "CurveRightEndPage${page}${region}ChangedPixelsfrom${i}to${j}"
            $addr       = "CurveRightEndPage${page}${region}Addressesfrom${i}to${j}"
            $instr      = "CurveRightEndPage${page}${region}WriteInstructionsfrom${i}to${j}"

            Add-Content -Path $outputFile -Encoding ascii -Value "`t${blockLabel}:	equ CurveRightEndBlock${blockNum}"
            Add-Content -Path $outputFile -Encoding ascii -Value "`t${prefix}:"
            Add-Content -Path $outputFile -Encoding ascii -Value "`tincbin ""..\grapx\RacingGame\CurveRightEndAnimationPage${page}\${region}changedpixelsfrom${i}to${j}.bin"""
            Add-Content -Path $outputFile -Encoding ascii -Value "`t${addr}:"
            Add-Content -Path $outputFile -Encoding ascii -Value "`tincbin ""..\grapx\RacingGame\CurveRightEndAnimationPage${page}\${region}addressesfrom${i}to${j}.bin"""
            Add-Content -Path $outputFile -Encoding ascii -Value "`t${instr}:"
            Add-Content -Path $outputFile -Encoding ascii -Value "`tincbin ""..\grapx\RacingGame\CurveRightEndAnimationPage${page}\${region}writeinstructionsfrom${i}to${j}.bin"""
