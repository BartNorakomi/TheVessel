...
for ($index = 1; $index -lt 23; $index++) {
    $next = $index + 1

    $image1Path = "{0}b.SR5" -f $index
    $image2Path = "{0}b.SR5" -f $next

    $changedPixelsPath = "changedpixels{0}b{1}b.bin" -f $index, $next
    $addressesPath = "addresses{0}b{1}b.bin" -f $index, $next
    $outputInstructionsPath = "writeinstructions{0}b{1}b.bin" -f $index, $next
