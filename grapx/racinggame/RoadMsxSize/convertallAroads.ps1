...
for ($index = 1; $index -lt 25; $index++) {
    $next = $index + 1

    $image1Path = "{0}a.SR5" -f $index
    $image2Path = "{0}a.SR5" -f $next

    $changedPixelsPath = "changedpixels{0}a{1}a.bin" -f $index, $next
    $addressesPath = "addresses{0}a{1}a.bin" -f $index, $next
    $outputInstructionsPath = "writeinstructions{0}a{1}a.bin" -f $index, $next
