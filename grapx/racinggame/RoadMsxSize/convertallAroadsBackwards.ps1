...
for ($index = 23; $index -gt 1; $index--) {
    ...
    $image1Path = "{0}a.SR5" -f $index      # CURRENT image
    $image2Path = "{0}a.SR5" -f $prev       # PREVIOUS image

    $changedPixelsPath = "changedpixels{0}a{1}a.bin" -f $index, $prev
    $addressesPath = "addresses{0}a{1}a.bin" -f $index, $prev
    $outputInstructionsPath = "writeinstructions{0}a{1}a.bin" -f $index, $prev
