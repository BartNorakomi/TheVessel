$outputFile = "CurveUpEndDataFiles.asm"
$startFrame = 1
$endFrame = 59
$RomBlockSize = 16KB
...
function Write-BlockHeader($blockNum) {
    @"
CurveUpEndBlock${blockNum}:  			equ   (`$-RomStartAddress) and (romsize-1) / RomBlockSize
	phase 0x8000
...
for ($i = $startFrame; $i -le $endFrame; $i++) {
    ...
    foreach ($page in 0, 1) {
        foreach ($region in "TOP", "BOTTOM") {
            # Build file paths
            $changedPixelsFile = ".\CurveUpEndAnimationPage${page}\${region}changedpixelsfrom${i}to${j}.bin"
            $addressesFile     = ".\CurveUpEndAnimationPage${page}\${region}addressesfrom${i}to${j}.bin"
            $writeInstrFile    = ".\CurveUpEndAnimationPage${page}\${region}writeinstructionsfrom${i}to${j}.bin"
            ...
            $blockLabel = "BlockCurveUpEndPage${page}${region}from${i}to${j}"
            $prefix     = "CurveUpEndPage${page}${region}ChangedPixelsfrom${i}to${j}"
            $addr       = "CurveUpEndPage${page}${region}Addressesfrom${i}to${j}"
            $instr      = "CurveUpEndPage${page}${region}WriteInstructionsfrom${i}to${j}"

            Add-Content -Path $outputFile -Encoding ascii -Value "`t${blockLabel}:	equ CurveUpEndBlock${blockNum}"
            Add-Content -Path $outputFile -Encoding ascii -Value "`t${prefix}:"
            Add-Content -Path $outputFile -Encoding ascii -Value "`tincbin ""..\grapx\RacingGame\CurveUpEndAnimationPage${page}\${region}changedpixelsfrom${i}to${j}.bin"""
            Add-Content -Path $outputFile -Encoding ascii -Value "`t${addr}:"
            Add-Content -Path $outputFile -Encoding ascii -Value "`tincbin ""..\grapx\RacingGame\CurveUpEndAnimationPage${page}\${region}addressesfrom${i}to${j}.bin"""
            Add-Content -Path $outputFile -Encoding ascii -Value "`t${instr}:"
            Add-Content -Path $outputFile -Encoding ascii -Value "`tincbin ""..\grapx\RacingGame\CurveUpEndAnimationPage${page}\${region}writeinstructionsfrom${i}to${j}.bin"""
