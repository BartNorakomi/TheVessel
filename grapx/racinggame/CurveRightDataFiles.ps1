$outputFile = "CurveRightDataFiles.asm"
$startFrame = 0
$endFrame = 75
$RomBlockSize = 16KB
...
function Write-BlockHeader($blockNum) {
    @"
CurveRightBlock${blockNum}:  			equ   (`$-RomStartAddress) and (romsize-1) / RomBlockSize
	phase 0x8000
...
for ($i = $startFrame; $i -lt $endFrame; $i++) {
    ...
    foreach ($page in 0, 1) {
        foreach ($region in "TOP", "BOTTOM") {
            # Build file paths
            $changedPixelsFile = ".\CurveRightAnimationPage${page}\${region}changedpixelsfrom${i}to${j}.bin"
            $addressesFile     = ".\CurveRightAnimationPage${page}\${region}addressesfrom${i}to${j}.bin"
            $writeInstrFile    = ".\CurveRightAnimationPage${page}\${region}writeinstructionsfrom${i}to${j}.bin"
            ...
            $blockLabel = "BlockCurveRightPage${page}${region}from${i}to${j}"
            $prefix     = "CurveRightPage${page}${region}ChangedPixelsfrom${i}to${j}"
            $addr       = "CurveRightPage${page}${region}Addressesfrom${i}to${j}"
            $instr      = "CurveRightPage${page}${region}WriteInstructionsfrom${i}to${j}"

            Add-Content -Path $outputFile -Encoding ascii -Value "`t${blockLabel}:	equ CurveRightBlock${blockNum}"
            Add-Content -Path $outputFile -Encoding ascii -Value "`t${prefix}:"
            Add-Content -Path $outputFile -Encoding ascii -Value "`tincbin ""..\grapx\RacingGame\CurveRightAnimationPage${page}\${region}changedpixelsfrom${i}to${j}.bin"""
            Add-Content -Path $outputFile -Encoding ascii -Value "`t${addr}:"
            Add-Content -Path $outputFile -Encoding ascii -Value "`tincbin ""..\grapx\RacingGame\CurveRightAnimationPage${page}\${region}addressesfrom${i}to${j}.bin"""
            Add-Content -Path $outputFile -Encoding ascii -Value "`t${instr}:"
            Add-Content -Path $outputFile -Encoding ascii -Value "`tincbin ""..\grapx\RacingGame\CurveRightAnimationPage${page}\${region}writeinstructionsfrom${i}to${j}.bin"""
