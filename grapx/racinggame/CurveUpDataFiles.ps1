$outputFile = "CurveUpDataFiles.asm"
$startFrame = 0
$endFrame = 59
$RomBlockSize = 16KB
...
function Write-BlockHeader($blockNum) {
    @"
CurveUpBlock${blockNum}:  			equ   (`$-RomStartAddress) and (romsize-1) / RomBlockSize
	phase 0x8000
...
for ($i = $startFrame; $i -lt $endFrame; $i++) {
    ...
    foreach ($page in 0, 1) {
        foreach ($region in "TOP", "BOTTOM") {
            # Build file paths
            $changedPixelsFile = ".\CurveUpAnimationPage${page}\${region}changedpixelsfrom${i}to${j}.bin"
            $addressesFile     = ".\CurveUpAnimationPage${page}\${region}addressesfrom${i}to${j}.bin"
            $writeInstrFile    = ".\CurveUpAnimationPage${page}\${region}writeinstructionsfrom${i}to${j}.bin"
            ...
            $blockLabel = "BlockCurveUpPage${page}${region}from${i}to${j}"
            $prefix     = "CurveUpPage${page}${region}ChangedPixelsfrom${i}to${j}"
            $addr       = "CurveUpPage${page}${region}Addressesfrom${i}to${j}"
            $instr      = "CurveUpPage${page}${region}WriteInstructionsfrom${i}to${j}"

            Add-Content -Path $outputFile -Encoding ascii -Value "`t${blockLabel}:	equ CurveUpBlock${blockNum}"
            Add-Content -Path $outputFile -Encoding ascii -Value "`t${prefix}:"
            Add-Content -Path $outputFile -Encoding ascii -Value "`tincbin ""..\grapx\RacingGame\CurveUpAnimationPage${page}\${region}changedpixelsfrom${i}to${j}.bin"""
            Add-Content -Path $outputFile -Encoding ascii -Value "`t${addr}:"
            Add-Content -Path $outputFile -Encoding ascii -Value "`tincbin ""..\grapx\RacingGame\CurveUpAnimationPage${page}\${region}addressesfrom${i}to${j}.bin"""
            Add-Content -Path $outputFile -Encoding ascii -Value "`t${instr}:"
            Add-Content -Path $outputFile -Encoding ascii -Value "`tincbin ""..\grapx\RacingGame\CurveUpAnimationPage${page}\${region}writeinstructionsfrom${i}to${j}.bin"""
